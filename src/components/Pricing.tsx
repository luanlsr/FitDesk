...
export default function Pricing() {
  return (
    <section id="precos" className="bg-[#111114] border-t border-[#222228] py-20 px-[5%]">
      <div className="text-center mb-16">
        ...
        <h2 className="font-bebas text-[clamp(2.5rem,5vw,4rem)] text-[#F5F5F0] leading-[1] mb-6">
          Escolha o plano <span>ideal</span> para você
        </h2>
        <p className="text-[#CFCFC8] text-base mb-8 max-w-[500px] mx-auto">
          Preços simples e transparentes. Teste grátis por 14 dias sem cartão de crédito.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
        {plans.map((plan, i) => (
          <div
            ...
            } rounded-2xl p-8 relative transition-all hover:-translate-y-1.5`}
          >
            {plan.featured && (
              <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-[#FF5C00] text-white px-4 py-1 rounded-full text-[0.72rem] font-bold tracking-[0.05em] uppercase whitespace-nowrap">
                Melhor Escolha
              </div>
            )}
            <div className="font-bebas text-2xl text-[#F5F5F0] mb-1 tracking-wider uppercase">{plan.name}</div>
            <div className="text-[0.82rem] text-[#7A7A85] mb-6 uppercase tracking-tight">{plan.desc}</div>
            
            <div className="flex items-end gap-1 mb-8">
              <div className="text-[0.9rem] text-[#7A7A85] mb-2 font-mono">R$</div>
              <div className="font-bebas text-6xl text-[#F5F5F0] leading-none mb-1">{plan.price}</div>
              <div className="text-[0.8rem] text-[#7A7A85] mb-2 font-mono">/mês</div>
            </div>

            <ul className="space-y-4 mb-8">
              {plan.features.map((f, j) => (
                <li key={j} className="flex items-center gap-3 text-[0.85rem] text-[#F5F5F0]">
                  {f.ok ? (
                    <Check className="w-4 h-4 text-[#00E676] flex-shrink-0" />
                  ) : (
                    <X className="w-4 h-4 text-[#222228] flex-shrink-0" />
                  )}
            ...
            <Link
              href="/signup"
              className={`block w-full text-center py-3.5 rounded-lg text-[0.9rem] font-bold no-underline transition-all ${
                plan.featured
